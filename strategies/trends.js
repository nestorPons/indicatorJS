...
function CLOSE_ORDER(order, candle){
    ...
    if(order.side == SELL){
        ...
        //if(isGreater(st.ema3.dist,1) && isGreater(-0.1,st.ema1.acc)) return true
        
        return true 
            && isGreater(0,st.ema1.acc)
            && isGreater(0.01,st.ema2.acc)
            && isGreater(0.001,st.ema3.acc)
...
function OPEN_ORDER_LONG(order, candle){
    ...
    return true
    && isGreater(st.ema1.val, st.ema4.val)
    && isGreater(1.5, st.ema4.dist)
    && isGreater(st.ema1.acc, 0.01)
    && isGreater(st.ema4.acc, 0.01)   
}
async function test(symbol, amount, candle, createOrder = false, isTest = true) {
    ...
    if (CLOSE_ORDER(orderSELL, candle)){
        ...
        cl = orderSELL
        console.log('CLOSE_SHORT =>', candle.time, orderSELL.profit.total)
        orderSELL = null
    } 
    if (CLOSE_ORDER(orderBUY, candle)){
        orderBUY.closePosition(st.closePrice, candle.time)
        cl = orderBUY
        console.log('CLOSE_LONG =>', candle.time, orderBUY.profit.total)
        orderBUY = null
    } 
    if(createOrder){  
        // ESTRATEGIA DE LARGO ---------------------------------      
        if (OPEN_ORDER_LONG(orderBUY, candle)){
            ...
            console.log('\x1b[31m%s\x1b[0m','OPEN_LONG=>',candle.time,'\x1b[31m%s\x1b[0m',st.ema4.dist)
            op = orderBUY
        }
        // ESTRATEGIA DE CORTO ------------------------------------------------
        if (OPEN_ORDER_SHORT(orderSELL, candle)){
            ...
            console.log('\x1b[31m%s\x1b[0m','OPEN_SHORT=>',candle.time,'\x1b[31m%s\x1b[0m',st.ema4.dist)
            op = orderSELL
        }
    }
